...
OUTPUT_DIR = "finetuned_model_by_sinzore"
NUM_EPOCHS = 10
BATCH_SIZE = 1
...
def replace_linear_with_lora(module: nn.Module, r=R, alpha=ALPHA, dropout=0.0, target_linear_names=None):
    for name, child in list(module.named_children()):
        replaced_child = replace_linear_with_lora(child, r=r, alpha=alpha, dropout=dropout, target_linear_names=target_linear_names)
        setattr(module, name, replaced_child)
    if isinstance(module, nn.Linear):
        if (target_linear_names is None) or any(t in module._get_name().lower() for t in target_linear_names):
            in_features = module.in_features
            out_features = module.out_features
            bias = module.bias is not None
            lora_linear = LoRALinear(in_features=in_features, out_features=out_features, r=r, alpha=alpha, dropout=dropout, bias=bias)
            with torch.no_grad():
...
def transcribe_audio_files():
    ...
    transcriber = pipeline("automatic-speech-recognition", model=TRANSCRIPTION_MODEL, device=-1)  # CPU
    audio_text_pairs = []
...
def prepare_csm_model_for_training():
    ...
    try:
        codebook_0_centroids = mimi.quantizer.codebooks[0].weight.data
    ...
    for name, param in model.named_parameters():
        ...
        elif "backbone.blocks.23" in name or "embedding" in name:
            param.requires_grad = True
        else:
...
def compute_loss_for_codebooks_single_pass(backbone_out, decoder_out, model, target_tokens, target_masks, device):
    ...
    audio_positions = target_masks[..., :-1].any(dim=-1)
...
def single_pass_forward(model, bridging_module, target_tokens, target_masks, positions):
    ...
    B, S, D = decoder_input.shape
    decoder_input_flat = decoder_input.view(-1, D).unsqueeze(1)
    decoder_out_flat = model.decoder(decoder_input_flat).to(dtype)
    decoder_out = decoder_out_flat.view(B, S, -1)
...
def calculate_validation_loss(model, bridging_module, dataset, device, max_samples=50):
    ...
    with torch.no_grad():
        for batch in val_loader:
            ...
            loss = single_pass_forward(model, bridging_module, target_tokens, target_masks, positions)
...
def remove_lora_modules(module: nn.Module) -> nn.Module:
    ...
    if isinstance(module, LoRALinear):
        ...
        new_linear = nn.Linear(in_features=in_features, out_features=out_features, bias=has_bias)
...
def finetune(model, dataset):
    ...
    trainable_params = list(p for p in model.parameters() if p.requires_grad) + list(bridging_module.parameters())
    ...
    global_step = 0
    validation_frequency = max(1, steps_per_epoch // 2)
    model.train()
    bridging_module.train()
